...
//[ 20120319 PZ1949 from KBJ need to check it 
...
#define AUTO_CHARGING_VMAXSEL				4220//4200 // 20120319 PZ1949 from KBJ
...
struct pm8058_charger {
	...
	struct pm8058_chip *pm_chip; //20120319 PZ1949 from KBJ
	struct device *dev;
...
static int m_chager_current = 0; //20120319 PZ1949 from KBJ
...
void Pm8058_printDump(void)  //20120319 PZ1949 from KBJ
{
	__dump_chg_regs();
}
EXPORT_SYMBOL(Pm8058_printDump);
...
static int pm_chg_imaxsel_set(int chg_current)
{
	...
	int diff;
		...
		m_chager_current = chg_current;//20120319 PZ1949 from KBJ
#endif  //CONFIG_SKY_CHARGING
...
#ifdef CONFIG_SKY_CHARGING  //kobj 110513
int pm8058_chg_get_current(void)  //20120319 PZ1949 from KBJ
{
	return m_chager_current;
}
EXPORT_SYMBOL(pm8058_chg_get_current);
...
{      //20120319 PZ1949 from KBJ  Blocked beneath 2 lines
...
extern void msm_charger_set_init_done(void);
#endif 
...
{
	int ret;
	int batt_vaild_result; //20120319 PZ1949 from KBJ  
	//dev_info(pm8058_chg.dev, "%s [SKY KOBJ]\n", __func__);	
	batt_vaild_result=1; //20120319 PZ1949 from KBJ  
...
//p11771_20111115_check batt id   //20120319 PZ1949 from KBJ  
			if (pm8058_is_battery_id_valid() == 0)
			{
				if (pm8058_is_battery_id_valid() == 0)
				{
					batt_vaild_result=0;
...
//pz1946 20111002 charging status only batt id check
			if(smb_charger_state == CHG_TYPE_NONE || smb_charger_state == CHG_TYPE_FACTORY) {
				...
				if (batt_vaild_result == 0 )  //20120319 PZ1949 from KBJ   Power off confition added
					kernel_power_off();
			}
		}
#else
...
//#endif  //CONFIG_SKY_CHARGING
	...
	return 0;
#endif  //CONFIG_SKY_CHARGING //20120319 PZ1949 from KBJ  
}
...
#ifdef CONFIG_SKY_CHARGING  //20120319 PZ1949 from KBJ       //p14682 kobj 110704
#define FACTORY_ID_MIN 2500 //20120516 PZ1949   //2100    //1800	 //900	
#define FACTORY_ID_MAX 4000 //20120516 PZ1949   //3100   //4800  //4400 //1600
#endif

#if defined(CONFIG_SKY_CHARGING) || defined(CONFIG_SKY_SMB_CHARGER)
extern unsigned int sky_raw_check_factory(void);
int pm8058_is_factory_cable(void)
{
	int cable_mv;
#ifdef CONFIG_SKY_CHARGING
